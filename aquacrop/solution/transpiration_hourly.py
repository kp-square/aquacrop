...
def transpiration_daily_setup_update_parameters(
    ...
    growing_season: bool,
    gdd: float,
) -> Tuple[float,float,float,"InitialCondition"]:
    ...
    if growing_season:
        ...
        ## Calculate potential transpiration parameters ##
    ...
    return Kcb_NS, Kcb, KsCold, NewCond


def transpiration_hourly_setup(
    Soil_Profile: "SoilProfile",
    Soil_nComp: int,
    Soil_zTop: float,
    Crop: "CropStructNT",
    ...
    if growing_season:
        ...
        class_args = {key: value for key, value in thRZ.__dict__.items() if
...
def transpiration_post_daily(NewCond, TrAct, TrPot0, growing_season):
    if growing_season:
